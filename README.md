# UXsyncNow
[UXstorm](http://UXstorm.com) is an independent software vendor that specializes in applications on the ServiceNow platform.  We have several applications on ServiceNow and are continually developing new ones.   In all our development, we do a lot of development with ServiceNow scripts/HTML/XML  This has lead us to creating this tool.  We hope it's as useful to you as it is to us.  If you wish to support us, check out  our User Experience applications on the Store and spread the word of our works!  If you are going to the ServiceNow Knowledge event, we will have a booth there so stop on by.
...
Download the UpdateSet from [ServiceNow Share](https://developer.servicenow.com/app.do#!/share/contents/1712544_uxsyncnow_uxstorms_servicenow_file_synchronizerdevelopment_tool).  Load the update set into your instance.  **NOTE**: This application is designed for development and should only be used in Development and Test, do not use this on a production instance.
